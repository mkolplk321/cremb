...
Page({
  ...
  get_product_collect: function() {
    ...
    app.baseGet(app.U({
      c: 'store_api',
      a: 'get_product_collect',
      q: {
        product_id: that.data.id
      }
    }), function(res) {
      that.setData({
        'storeInfo.userCollect': res.data.userCollect
      });
    });
  },
  /**
  ...
  setCollect: function() {
    if (app.globalData.isLog === false) {
      ...
      var url = app.U({
          ...
          q: {
            productId: this.data.storeInfo.id
          }
        }),
        that = this;
  ...
  joinCart: function() {
    ...
    else if (this.data.storeInfo.is_vip && this.data.sharePacket.isState && app.globalData.spid === 0)
      app.Tips({
        title: '您还不是经销商，此创业商品仅能经销商或通过经销商购买，请联系合伙人先成为经销商！'
      });
    else
      this.goCat();
  },
  ...
  goBuy: function() {
    ...
    if (app.globalData.isLog === false)
      ...
      app.Tips({
        title: '您还不是经销商，此创业商品仅能经销商或通过经销商购买，请联系合伙人获取经销商推广链接！'
      });
    else
      this.goCat(true);
  },
